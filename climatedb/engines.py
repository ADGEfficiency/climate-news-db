    ...
    def get(self):
        ...
        for f in self.fldr.iterdir():
            if f.is_file() and f.suffix == ".html":
                with open(f, "r") as fi:
                    data.append(
                        {self.key: f.with_suffix("").name, self.value: fi.read()}
                    )
        return data
...
class SQLiteEngine(AbstractDB):
    def __init__(
        ...
        qry = (
            f"CREATE UNIQUE INDEX IF NOT EXISTS 'index' ON {self.table} ({self.index});"
        )
        self.c.execute(qry)
    ...
    def exists(self, key, value):
        data = self.c.execute(
            f"SELECT * FROM {self.table} WHERE {key}=?", (value,)
        ).fetchall()
        if data:
            return True
